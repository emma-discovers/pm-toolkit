...
- **Data inventory:** for each dataset, record source, fields, PII classification, rights/basis, retention period, residency, and access controls.
...
- **Human-in-the-loop:** who reviews, when, and what they can override. Never write AI-generated data straight to a production system of record without review. Specify UX flows that let the user edit, regenerate, or reject the output before it's committed.
- **Explainability:** UI components that show why the model produced an answer — for example, citing the specific source document in a RAG system.
- **Refusal policy:** what should the model never do, what should it refuse, what should it redirect, and what should it answer plainly. "Don't be harmful" is not a refusal policy. A real policy lists concrete categories with examples.
...
- **Quality metrics:** acceptance rate (target ≥90%), hallucination rate (target ≤2% by faithfulness rubric), refusal rate on benign queries (target ≤4%), refusal rate on harmful queries (target ≥98%).
- **Human evaluation:** rubric (1–5) with definitions, reviewer sampling and calibration plan, inter-rater agreement target.
...
- **What happens if the model produces a harmful or incorrect output?** Escalation path, not just an error message. Human-in-the-loop design, not error handling.
- **What happens if the model is slow?** Stream the output. Token-by-token streaming dramatically improves perceived latency even when total generation time is unchanged.
...
- **Cost per interaction:** estimated monthly cost at scale, based on token pricing. Output tokens typically cost 5–6x input tokens.
### 7. Rollout plan
...
- **Rollout ramp:** shadow → internal → canary → percentage → GA, with gates at each stage.
...
1. **Hallucination-tolerance hand-waving.** "We will work hard to minimise hallucinations" is not a requirement. Name a number.
2. **No eval criteria before model selection.** Picking a model before defining what you're evaluating means you have no defensible reason for the choice.
3. **No fallback model.** Frontier-model outages happen. Vendor pricing changes happen. A single-vendor PRD bakes in operational risk that nobody named.
4. **"Accurate" without definition.** Accuracy is statistical, not categorical. Define the unit, evaluator, population, and consequence of error.
5. **Guardrails as an afterthought.** Guardrails are the result of having failed to encode requirements earlier in the system design. Runtime guardrails are almost always evidence that requirements weren't encoded upstream.
